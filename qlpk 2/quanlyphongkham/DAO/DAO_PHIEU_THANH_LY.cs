...
namespace quanlyphongkham.DAO
{
    class DAO_PHIEU_THANH_LY
    {
        ...
        public DataTable Get_PN_TheoNgay(string tungay, string denngay)
        {
            ...
            string fromday = tungay.Substring(0, 10);
            string today = denngay.Substring(0, 10);
            fromday =  fromday + " 00:00:00";
            today = today + " 23:59:59";

            cmd.Parameters["@tungay"].Value = DateTime.ParseExact(fromday, "dd/MM/yyyy HH:mm:ss", null);
            cmd.Parameters["@denngay"].Value = DateTime.ParseExact(today, "dd/MM/yyyy HH:mm:ss", null);
            con.Open();
        ...
        public DataTable get_VatTuTon(string ten_vt)
        {
            SqlConnection con = new SqlConnection(db.connectionStr);
            SqlCommand cmd = new SqlCommand("get_VatTuTon", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@ten_vt", SqlDbType.NVarChar,150);

            cmd.Parameters["@ten_vt"].Value = ten_vt;
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
        ...
        public DataTable get_VatTuTheoKho(string kho)
        {
            SqlConnection con = new SqlConnection(db.connectionStr);
            SqlCommand cmd = new SqlCommand("getVatTuTheoKho", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@kho_id", SqlDbType.NVarChar, 20);

            cmd.Parameters["@kho_id"].Value = kho;
            con.Open();
            SqlDataAdapter da = new SqlDataAdapter(cmd);
            DataTable dt = new DataTable();
            da.Fill(dt);
