...
namespace quanlyphongkham.DAO
{
    class DAO_TOA_THUOC
    {
        ...
        public int getTTPK_ID_MAX_by_KBID(string kbid)
        {
            try
            {
                SqlConnection conn = new SqlConnection(connecDB.connectionStr);
                SqlCommand cmd = new SqlCommand("getTTPK_ID_MAX_by_KBID", conn);
        ...
        public DataTable getTTPKbyBN(string idbn)
        {

            SqlConnection conn = new SqlConnection(connecDB.connectionStr);
            SqlCommand cmd = new SqlCommand("getTTPKbyBN", conn);
            DataTable dt = new DataTable();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.Add("@BN_ID", SqlDbType.NVarChar, 15).Value = idbn;
            conn.Open();
