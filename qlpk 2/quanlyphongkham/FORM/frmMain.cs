...
namespace quanlyphongkham.FORM
{
    public partial class frmMain : Form
    {
        ...
        XtraTabPage tabXT;
        frmXUATTHUOC frmXT;
        private void btnXuatThuoc_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (frmXT == null)
            {
                tabXT = new XtraTabPage();
                frmXT = new frmXUATTHUOC();
                addForm(tabXT, frmXT, "Xuất thuốc");
            }
            else
            {
                loadPage(tabXT, frmXT);
            }
        }

        XtraTabPage tabThanhLy;
        frmPHIEU_THANH_LY frmPTL;
        private void btnThanhLy_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            if (frmPTL == null)
            {
                tabThanhLy = new XtraTabPage();
                frmPTL = new frmPHIEU_THANH_LY();
                addForm(tabThanhLy, frmPTL, "Thanh lý vật tư");
            }
            else
            {
                loadPage(tabThanhLy, frmPTL);
            }
        }
    }
}
